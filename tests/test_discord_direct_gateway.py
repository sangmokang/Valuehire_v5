...
class MinimalPrivilegeQueueClientRpcOnlyTests(unittest.TestCase):
    ...
    def test_enqueue_rejects_database_machine_identity_drift(self) -> None:
        client, urls, _fake_urlopen = self._client_with_recorder()

        class FakeHTTPResponse:
            def read(self) -> bytes:
                return (
                    b'[{"id":1,"status":"queued","machine":"macbook_pro"}]'
                )
        ...
        with patch(
            ...
            side_effect=lambda _req, timeout=30: FakeHTTPResponse(),
        ), patch(
            "tools.multi_position_sourcing.job_queue.url_host_resolves_public",
            return_value=True,
        ):
