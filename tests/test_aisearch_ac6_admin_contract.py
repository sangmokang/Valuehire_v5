"""AC-6 — admin 등록 계약 검증(PR#250 원본 테스트 복원, 2026-07-31 V1 2라운드).

이 파일은 PR#250 의 tests/test_aisearch_ac6_admin_api.py 를 **정본 모듈
(admin_api_client.py)** 기준으로 되살린 것이다. admin_api.py 를 지우면서 이
테스트도 함께 사라졌는데, V1 독립 적대검증이 "보호장치를 이식했다면 그 반례
테스트도 같이 있어야 한다"고 지적했다(테스트 약화). 선택 필드 형식·악성 응답
객체·개인정보 비노출·극단 상태값 반례가 여기서 다시 지켜진다.
...
class TestParseRegisterResponse:
    ...
    def test_describe_is_total_even_when_len_explodes(self):
        ...
        from apps.aisearch.core.admin_api_client import _describe
    ...
    def test_internal_defect_is_not_disguised_as_a_response_error(self, monkeypatch):
        ...
        import apps.aisearch.core.admin_api_client as module
