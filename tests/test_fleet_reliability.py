...
class TestPauseCooldown:
    def test_paused_후_쿨다운_시간(self):
        ...
        assert PAUSE_COOLDOWN_SECONDS == 180, "#107 — 전체 워커 양보는 정확히 3분"
    ...
    def test_loop_가_paused_후_실제로_쉰다(self, monkeypatch):
        ...
        assert sleeps[0] == 180, "일시정지 직후 180초 양보 없이 재claim 금지"

    def test_restart_preserves_remaining_yield_without_auto_resuming_security_job(
            self, tmp_path):
        """저장값은 벽시계 epoch로 남고, 재기동 후엔 남은 시간만 기다린다."""
        mono = [10.0]
        ...
        class Q:
            ...
            def release(self, job_id, status, **kwargs):
                self.released.append((job_id, status))
                return {}

        paused_job = {"id": 1, "skill": "humansearch", "machine": "macmini",
                      "account_key": "portal:paused",
                      "position_url": "https://x.co/p", "requested_by": "u",
                      "role": "owner", "params": {}}
        q1 = Q([paused_job])
        w1 = FleetWorker(
            "macmini", queue=q1,
            runner=lambda p, t: ("PAUSED_FOR_HUMAN: 2FA", 0),
            notifier=lambda j, t: None,
            clock=lambda: mono[0], wall_clock=lambda: wall[0],
            yield_state_path=state_path)
        assert w1.run_once() == "paused_for_human"
        saved = json.loads(state_path.read_text(encoding="utf-8"))
        assert saved["yield_until_epoch"] == wall[0] + 180

        wall[0] += 45
        restarted_mono = [500.0]
        next_job = {**paused_job, "id": 2, "account_key": "portal:other"}
        q2 = Q([next_job])
        w2 = FleetWorker(
            "macmini", queue=q2,
            runner=lambda p, t: ("정상 완료", 0),
            notifier=lambda j, t: None,
            clock=lambda: restarted_mono[0], wall_clock=lambda: wall[0],
            yield_state_path=state_path)
        assert w2.run_once() == "idle"
        assert q2.claims == 0, "재기동으로 남은 양보 시간을 우회하면 안 됨"
        assert w2._post_status_delay("idle", 30) == 135, "고정 180초가 아니라 남은 시간"

        restarted_mono[0] += 135
        wall[0] += 135
        assert w2.run_once() == "done"
        assert q1.released == [(1, "paused_for_human")], (
            "캡차·2FA 잡 자체는 180초 경과로 자동 resume 하지 않음")

    def test_production_state_path_is_under_valuehire_home(self):
        assert default_yield_state_path("macmini") == (
            Path.home() / ".valuehire" / "fleet" / "owner-yield-macmini.json")

    def test_corrupt_backlog_cannot_cancel_valid_restart_yield(self, tmp_path):
        state_path = tmp_path / "owner-yield-macmini.json"
        ...
        assert w._yield_remaining() == 120
        assert w._variant_backlog == []
        assert w.run_once() == "idle"
        assert q.claims == 0, "손상된 backlog가 유효한 3분 양보까지 우회하면 안 됨"
