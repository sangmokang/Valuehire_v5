...
계약 재사용·단일 파싱(INV-D3):
- 텍스트 인자 해석 = ``parse_fleet_args`` (fleet_args 로 이사, 재구현 금지)
...
기존 Hermes 어댑터와 달리 길드/채널/역할 컨텍스트를
...
from .fleet_args import FleetArgsError, parse_fleet_args
...
def handle_envelope(
    ...
    *,
    ...
    try:
        options = parse_fleet_args(envelope.command, envelope.raw_args)
