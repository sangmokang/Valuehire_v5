...
v5·v4 레포와 사용자 전역에 등록된 사장님 저작 스킬을 Codex CLI가 읽는
`~/.codex/skills/` 로 미러링한다. Codex도 `SKILL.md` 형식이 Claude와 동일하다.
...
- 이름 충돌 시 **먼저 온 소스**가 이김. 우선순위는 v5 `skills` → v5
  `.claude/skills` → v4 `.codex/skills` → v4 `.claude/skills` → v4 `tools` →
  `~/.claude/skills`.
- v4 루트 기본은 v5와 같은 부모의 `valuehire_v4`. 다른 머신은
  `VALUEHIRE_V4_REPO` 절대경로로 지정.
- 원본·대상 경로가 겹치면 쓰기 전에 거부. 스킬 최상위 심볼릭 링크도
  외부 폴더 유출·순환 참조를 막기 위해 복사하지 않음.
