...
저장소 밖의 필수 근거가 없으면 조용히 생략하거나 현재 기계 경로로 대체하지 마십시오. 휴대
가능한 저장소 입력으로 전환하는 선행 이슈를 먼저 병합하고, 그 전에는 제품 단위를 시작하지 마십시오.
...
- 두 번째 Mac에서도 같은 커밋으로 실행해 schema뿐 아니라 입력·증거·판정의 안정 해시를 대조하고,
