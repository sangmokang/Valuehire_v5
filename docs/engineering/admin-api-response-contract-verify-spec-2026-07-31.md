...
| S-RES-9 | **오류 메시지에 서버 원문 내용 금지.** 후보 개인정보(이름·연락처)가 로그·원장으로 새면 안 됨 — 타입·크기 등 '모양'만. 봉투(비-Mapping·bytes text)·`error` 필드·`deduped` 값 **전 경로**에 적용 |
| S-RES-11 | **성공 결과에 서버 응답 본문을 보관하지 않는다.** `RegisterOutcome` 은 안전한 영수증(`recorded`/`deduped`/`status`/`candidate_id`)만 남기고, 서버가 덧붙인 필드와 후보 개인정보는 결과에 눌러앉지 않는다 (관용 ≠ 보관) |
...
| # | 조항 |
|---|---|
| S-ROB-1 | `_describe_shape` 는 **어떤 입력에도 예외를 던지지 않는다** (`len()`·타입 이름 문자열화가 터지는 값 포함). repr 기반 `_describe` 는 그 자체가 개인정보 유출 경로여서 **제거**했다(S-RES-9) |
