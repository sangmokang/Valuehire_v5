...
>
> 버전: v1.2.0 · 2026-07-26
>
> 로그인 상태와 허용 조작은 `26-portal-login-spec@1.5.0`을 따릅니다.
...
| # | 단계 | 핵심 |
|---|---|---|
| 0 | 사전 점검 | v5에서만. 정책이 증명한 exact endpoint/profile/target 확인(포트·프로필 추측 금지) |
| 1 | 점유·로그인 게이트 | 탭 조회 → 채널별 {READY/OCCUPIED/HUMAN_AUTH/HANDOFF/AUTH_CONFLICT} 판정 |
...
| 6 | 적합도 평가 | `candidate-match-v2-2026-07-24`: LLM gate+D1~D8 근거 소점수, 코드 총점·상한·등급 |
...
- **INV4** 캡차·봇차단 감지 → 그 채널은 **HUMAN_AUTH**, 자동 우회 금지.
- **INV15** 로그인 증거 부족은 **HANDOFF**, 봉인된 다중 인증 증거는
  **AUTH_CONFLICT**로 구분하며 두 상태를 사람 인증 대기로 합치지 않는다.
...
- 로그인 상태·허용 조작: `26-portal-login-spec@1.5.0`
