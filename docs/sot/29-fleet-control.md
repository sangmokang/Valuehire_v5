...
> 로그인 상태·기기 충돌·사람 인증 판정은 `26-portal-login-spec@1.5.0`이 우선한다.
> 인증 기기 수 미증명은 `HANDOFF`, 2개 이상은 terminal `AUTH_CONFLICT`,
> 실제 캡차·2FA·checkpoint만 `HUMAN_AUTH`이며 시간 자동 재개 금지다.
...
- **LinkedIn 잡(skill=url)은 봉인된 함대 증거와 APP 17의 경로 결정이 정확히 한 기기와
  기존 탭을 증명한 경우에만 그 기기를 사용한다.** 인증 기기 수 미증명·조회 실패·정확 대상
  부재는 `HANDOFF`, 인증 기기 2개 이상은 terminal `AUTH_CONFLICT`다. 신뢰도 순 선택과
  `macmini` 기본값은 사용하지 않는다. 계정 글로벌 락(account_key)은 그대로라 같은 계정
  동시 2머신 실행은 여전히 불가능하다.
...
- **INV9 · 사장님 양보 60초(1분) 자동 재개 + 3사 포털 한정 (2026-07-20 사장님 지시로 개정, 원본 2026-07-15 #107)**: 사장님 개입으로 보는 것은 **크롬 활성 탭이 3사 포털(사람인·잡코리아·링크드인)일 때뿐**이다 — 유튜브 등 다른 화면 사용은 개입이 아니며 자동작업을 멈추지 않는다. 이 일반적인 소유자 활동은 마지막 신호로부터 **60초(1분)** 동안 이상이 없으면 자동작업을 재개한다. 다만 실제 캡차·2FA·checkpoint인 `HUMAN_AUTH`는 이 시간 재개 대상이 아니며, owner가 인증을 마친 뒤 명시적으로 `fleet-resume`해야 한다. `HANDOFF`와 `AUTH_CONFLICT`는 사람 대기로 저장하지 않고 종결한다.
