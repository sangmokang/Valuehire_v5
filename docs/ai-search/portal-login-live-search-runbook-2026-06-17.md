# 실행 금지 — 과거 라이브 서치 실행 기록

`historical_input_not_executable`

이 문서는 과거 실행 기록이며 현재 로그인 지시로 실행하면 안 됩니다. 현재 정책은
`26-portal-login-spec@1.5.0`입니다. LinkedIn 폼 로그인과 범용 자동 복구 주장은 폐기됐으며,
APP 17·APP 30/31의 `LINKEDIN_LI_AT` 결정표만 유효합니다.
