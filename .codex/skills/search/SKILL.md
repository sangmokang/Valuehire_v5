...
> **SOT 재확인**: 캡차·2FA·IP보안 같은 **보안 챌린지는 절대 자동 우회하거나 반복 시도하지 않는다.** 막히면 멈추고 사람이 해결한다(검색엔진이 막히면 Brave/Bing으로 전환). 사람인·잡코리아·LinkedIn 같은 보호 포털의 로그인·세션 자동 복구(재로그인 백오프·검색 시간제한·셀렉터 드리프트 감지·크롬 잔재 정리)는 이 Skill이 아니라 **`multisearch` Skill**이 담당한다 — 포털 소싱 안정성은 그쪽 "운영 안정성(자동 복구)" 절을 따른다.
...
1. **사람 개입 대기를 시간가드로 자르지 마라.** 로그인 preflight의 channel timeout이 human-intervention 대기보다 짧으면 사장님이 로그인하는 도중 창이 강제 종료된다(2026-06-17 실제 발생). 사람 개입이 켜져 있으면 channel timeout은 사람 timeout보다 길거나 `0`(비활성)이어야 한다.
2. **로그인 성공 후 세션 유지.** 사람인/잡코리아 영속 프로필 쿠키는 `~/.valuehire/portal_profiles`에 디스크 저장되어 다음 실행에 재사용된다(재로그인 강요 금지). 링크드인은 CDP(사장님 크롬 9222) attach라 연결만 끊고 창은 닫지 않는다 — 사람인·잡코리아 창도 유지하려면 같은 프로세스에서 열고 `context.close()`를 호출하지 않는다.
