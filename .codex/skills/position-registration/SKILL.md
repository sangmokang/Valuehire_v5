...
- 관련 SOT: `docs/sot/`
- 이 스킬 본문과 포지션 등록 테스트
- 기존 구현 진입점: `tools/multi_position_sourcing/position_registration.py`
- 관련 파서·추출·인식·중복 코드: `request_parser.py`, `posting_extractor.py`, `posting_recognizer.py`, `position_dedup.py`
...
- 알람 폭탄 금지. 여러 포지션·여러 후보·여러 항목은 한 메시지 또는 한 댓글로 묶는다.
- 포지션 원본 URL, profile_url 등 필수 URL/필드는 쓰기 직전 무결성 검사를 통과해야 한다.
