...
이 스킬은 사람인, 잡코리아, LinkedIn Recruiter/RPS를 여는 모든 작업의 선행 절차다.
Claude, Codex, Hermes는 검색·프로필 열람·포지션 등록보다 먼저 이 스킬을 적용한다.

정본:
- 로그인 판정과 차단 신호: `docs/sot/26-portal-login-spec.json`
- 사람 개입과 창 보존: `docs/ai-search/portal-login-live-search-runbook-2026-06-17.md`
...
6. 호환되는 브라우저 프로세스 자체가 없을 때만 `./scripts/portal_browsers.sh start`를 한 번 실행한다. 이 스크립트의 프로세스/프로필 중복 가드를 우회하지 않는다.
...
`raw_cdp.attach()`가 화면 상단에 DOM id `vh-automation-badge`를 주입해야 한다. 배지 문구에는 실행 주체와 작업을 표시한다. 배지는 클릭을 가리지 않아야 하며 페이지 이동 후 다시 나타나야 한다.
...
3. 로그아웃 채널만 모아 검증된 실행기를 한 번 호출한다.
4. 자격증명 자동 제출 후 로그인 마커를 기다린다.
5. captcha/2FA/checkpoint/이상 접근/세션 충돌이면 즉시 `HUMAN_AUTH`로 바꾸고 제출·클릭을 멈춘다.
6. 사람이 해결하면 동일 탭·동일 프로필에서 마커를 재확인한다. 새 로그인 탭을 만들지 않는다.
...
python3 -m tools.multi_position_sourcing.portal_login \
  ...
  --worker-id default \
  --chrome-cdp-endpoint "<실제 LinkedIn CDP endpoint>" \
  --channel-timeout-seconds 0 \
  --human-timeout-seconds 1800 \
  --output artifacts/portal_session_status_latest.json
```

주의:
- `--channel-timeout-seconds 0`으로 사람 로그인 도중 상위 제한시간이 창을 정리하지 못하게 한다.
- 프로세스 관리자는 사람 개입 대기 프로세스를 임의로 종료하지 않는다.
- 직접 password를 로그나 명령행 인자로 넣지 않는다.
- `portal_sessions[].login`이 `existing_session_ok`, `auto_login_ok`, `human_intervention_ok` 중 하나이고 사이트별 마커가 확인되어야 준비 완료다.
...
- 성공: `/talent/` home/search/profile이 login-cap이나 enterprise-authentication으로 이동하지 않고 로드된다.
...
1. 마지막 로그인 확인 또는 keepalive 후 30분 이상 지났을 때만 검토한다. keepalive 간격은 최소 10분보다 짧아질 수 없다.
2. `HUMAN_ACTIVE`나 `HUMAN_AUTH`이면 하지 않는다. AI가 작업 중인 정확한 탭 하나에서만 한다.
3. 먼저 현재 페이지의 로그인 마커를 읽기 전용으로 확인한다. 살아 있으면 불필요한 클릭을 생략할 수 있다.
4. 사이트가 실제 페이지 전환을 요구할 때는 이미 이번 작업에서 정상적으로 열었던 프로필 상세 URL을 1회 방문한다. 새 후보를 소비하거나 유료 열람을 만들 수 있는 프로필은 열지 않는다.
5. 검증된 프로필 상세가 없으면 talent pool/home 같은 읽기 전용 안전 페이지를 1회 방문한다. 임의 후보를 클릭하지 않는다.
6. 방문 후 로그인 마커를 다시 확인하고 `last_verified_at`, `last_keepalive_at`, `session_age_seconds`를 갱신한다.
7. 팝업, 모달, 유료 차감, 저장, 제안, InMail, Send 버튼은 절대 누르지 않는다.
8. 로그인 화면으로 바뀌면 반복 새로고침하지 않고 `AUTH_LOST`로 전이한다.
