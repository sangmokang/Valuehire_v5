...
> **로그인 정책 재확인**: 보호 포털의 로그인 상태 판정은 `login`·`multisearch`가
> `26-portal-login-spec@1.5.0`에 따라 수행한다. 사람인·잡코리아는 저장 자격증명을 정확한 기존
> target 하나에 최대 1회만 입력하고, 대상이 하나로 증명되지 않으면 `HANDOFF`한다. LinkedIn은
> 인증 기기 수 미증명이면 `HANDOFF`, 1개면 무변경 재사용, 봉인된 조사로 인증 기기 2개 이상이면
> `AUTH_CONFLICT`다. 0개일 때만 소유자 승인과 `APP 17`의 동일 기기 target 1개 결정 뒤
> `APP 30/31`이 `LINKEDIN_LI_AT`을 최대 1회 적용하도록 결정할 수 있다. 캡차·2FA·IP보안은
> `HUMAN_AUTH`이며 자동 우회나 반복 시도를 하지 않는다.
...
> 로그인 판단의 현재 기준은 `26-portal-login-spec@1.5.0`이다.
> `docs/ai-search/portal-login-live-search-runbook-2026-06-17.md`는 과거 실행 기록일 뿐 현재
> 로그인 지시로 실행하지 않는다.

사장님이 visible browser에서 **직접 로그인하거나 보안챌린지를 해결하면, 그 로그인 세션을 절대 닫거나 버리지 않는다.** 한 번 들어온 로그인은 최대한 오래 살린다.

1. **사람 개입 대기를 시간가드로 자르지 마라.** `HUMAN_AUTH`에는 시간제한을 두지 않는다.
   양성 로그인 마커가 확인되고 15초 동안 사람 입력이 없는 때에만 같은 target에서 재개한다.
2. **로그인 성공 후 세션 유지.** 로그인 정책이 증명한 exact endpoint/profile/target만 재사용하고
   연결만 해제한다. 고정 포트를 단정하거나 다른 포트를 추측하지 않는다(포트 추측 금지).

## 라이브 수집 안정성 (검증된 교훈 2026-06-17)

- **LinkedIn Recruiter 결과는 JS 렌더가 느리다 — 검색 직후 1초만 기다리면 0건이 나온다(실제 발생).** `a[href*="/talent/profile/"]`가 뜰 때까지 최대 ~15초 대기 + 스크롤 후 수집한다. selector는 정상, 문제는 대기 부족이었다.
- 사람인 talent-pool은 `main/tutorial`로 빠지거나 인증 리다이렉트로 not_ready 오탐이 날 수 있다 — 기업회원 URL(`ut=c`)로만 진입하고 검색화면 마커 도달을 확인한다.
- 채널이 막혀 0건이면 "후보 없음"이 아니라 **"채널 제한으로 미확보"**로 보고한다(원인 단정 금지).
- 보내는 모든 profile URL은 **실제로 열어 이름이 페이지에 있는지 확인**한 것만 쓴다(URL 절대 오류 금지). 원시 결과는 직무·지역·연차로 **선별**해서만 내보낸다.
