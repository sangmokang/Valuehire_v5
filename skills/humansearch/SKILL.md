...
브라우저를 열거나 탭을 고르거나 CDP에 attach/navigate 하기 전에 실행 중인 모든 디버그
크롬의 실제 CDP 포트와 `--user-data-dir`를 확인하고, 각 엔드포인트 `/json/list`의
페이지 타입·제목·URL·로그인·캡차·멀티세션 신호를 전수 조사해 한국어로 보고한다.
이 상태 보고가 끝나기 전에는 브라우저 시작·검색·프로필 순회를 하지 않는다. 고정 포트
추정은 금지하며 조사한 프로필 헬퍼로 실제 포트를 다시 해석한다.

사장님이 잠시 크롬을 만지면 자동으로 양보한다. 손길이 사라진 뒤 짧은 유휴 확인을 통과하면
중단한 페이지·후보부터 자동 재개한다. 잠시 점유를 영구 중단으로 취급하지 않으며 같은 URL을
연속 재탐색하거나 창을 반복해서 열고 닫지 않는다. 캡차·2FA·멀티세션 락만 사람 게이트로 멈춘다.
...
4. 통과한 후보만 채점(`score_humansearch`): 학력30·직무50·논리10·이직안정10 → 0~100.
   - 직무50 안에서는 `position.organization_analysis` / `talent_density_notes` 를 같이 읽어
     스타트업·스케일업·엔터프라이즈 맥락을 보정한다.
   - 점수는 높아도 대형 조직/플랫폼 출신이면 `why_not` 에 코멘트를 남겨
     "현재 포지션 환경과의 간극"을 드러낸다.
   - `org_fit` 라벨을 별도 필드로 노출한다(`builder-fit` / `enterprise-fit` / `*_mismatch` / `neutral`).
   - 포지션 조직 분석은 sqlite 기본 저장소에 먼저 누적하고,
     `scripts/organization_analysis_supabase_backfill.py` 배치로 Supabase `organization_analysis` 테이블에 미러한다.
