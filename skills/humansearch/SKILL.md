---
name: humansearch
description: 사장님이 채용 사이트(LinkedIn Recruiter/RPS·사람인·잡코리아)에 검색을 직접 걸어둔 리스팅 화면(또는 RPS recruiterSearch URL)을 받아 후보 프로필을 순회하고, 필수요건 게이트와 근거 기반 D1~D8 점수로 평가해 합격 후보를 Discord #ai_search 또는 ClickUp FY26AI_Search에 등록한다. 검색어·필터는 입력하지 않는다. 트리거 — "humansearch", "휴먼서치", "이 화면 순회해서 후보 솎아줘", "리스팅 돌면서 채점해서 디스코드로", "이 RPS 검색결과로 후보 찾아 ClickUp 에 등록", "포지션 Task 만들고 Subtask 에 후보 리스팅".
---
...
- 관련 SOT: `docs/sot/`
- 매칭 프롬프트 정본: `docs/sot/24-position-jd-sot.json`
  (`candidate-match-v2-2026-07-24`)
...
4. 통과한 후보는 `candidate-match-v2-2026-07-24`의 Stage 1~3으로
   필수요건 게이트와 D1~D8 소점수를 만든다. LLM이 총점을 내거나 문장력·표현
   품질을 채점하면 무효다. Stage 4 결정 코드만 0~100 총점·등급을 계산한다.
