...
5. **Discord 보고.** 서치 절차 **중간 보고**(시작·페이지 단위)와 완료 보고를 Discord
   채널 `814353841088757800` 로 보낸다(봇 토큰 `DISCORD_BOT_TOKEN`). 채널 접근 불가(404 등)
   시 `VALUEHIRE_SEARCH_LIST_DISCORD_WEBHOOK_URL`(#ai_search) 폴백. 알람 폭탄 금지 —
