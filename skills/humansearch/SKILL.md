...
8. **핵심 후보 개인화 제안(InMail) 문구 작성 — 발송 아님 (2026-07-03, 골든샘플 기준).**
   ...
   - **언어 자동 선택 (2026-07-03)**: 프로필 이름·이력이 영문이면 본문 영어(인사말만 한국어 허용),
     한국어 프로필이면 한국어 — `body_language_for_profile()` 로 기계 판정.
   - ⛔ **발송 전 기계 체크리스트 통과 전 문구 제공 금지 (2026-07-03, Movensys 사고 봉인)**:
     ...
     요소(§1.5 8요소) 6개 미만이면 보고 후 진행 ⑤ 자모 분리·기지 오타 STOP + 컴포저 입력 후
     스크린샷 대조 ⑥ VERIFIED-PULL 문단 존재. 상세는 골든샘플 "발송 전 기계 체크리스트" 절.
